...
public class SCC_Camera : MonoBehaviour {
    ...
    private void LateUpdate() {
        ...
        if (Mathf.Abs(game.inputs.lookDelta.x) < 1 && Mathf.Abs(game.inputs.lookDelta.x) > 0)
            cameraRotation = 100.0f * game.inputs.lookDelta.x;

        // Mouse
        else if (Mathf.Abs(game.inputs.lookDelta.x) >= 1 && game.inputs.rightClick)
            cameraRotation = Mathf.Clamp(cameraRotation + (game.inputs.lookDelta.x / 10), -150.0f, 150.0f); 

        if (!game.inputs.rightClick && game.inputs.lookDelta.x == 0)
            cameraRotation = 0;

        //  Angles of the target vehicle and camera.
        wantedRotationAngle = playerCar.eulerAngles.y + cameraRotation;
